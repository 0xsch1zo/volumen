...
void timetable::timetable_display(ft::Component timetable_component, api *api,
  ...
  auto loading_screen = std::jthread([&](std::stop_token s_token) {
    ...
    auto loading_screen_component = ft::Renderer([&] {
      return ft::vbox({ft::separatorEmpty(),
                       ft::spinner(utils::SPINNER_TYPE, spin_counter) |
                           ft::flex | ft::center}) |
  ...
  const std::vector<std::string> weekdays = {"<",         "Monday",   "Tuesday",
  ...
  for (size_t i{}; i < weekdays.size() - ACTION_COUNT; i++) {
    auto lessons_component =
        ft::Container::Vertical({lessons(timetable_o.timetable[i], events_o)});
    timetable_container->Add(ft::Renderer(lessons_component, [=] {
      auto base =
          lessons_component->Render() | ft::vscroll_indicator | ft::yframe;
      if (!lessons_component->Focused())
        return base | ft::size(ft::HEIGHT, ft::LESS_THAN,
  ...
  // On first run
  if (url.empty()) {
    // WARNING: If you pass timetable_component directly you will create a
    // std::shared_ptr dependency cycly and with that a memory leak
    std::weak_ptr timetable_component_for_lamda = timetable_component;

    day_selector =
        utils::get_day_of_week(api->get_today()) +
        ACTION_PREV_ENTRY_OFFSET; // Offset by ACTION_PREV_ENTRY_OFFSET because
                                  // of action prev('<') element
    ft::Component menu =
        ft::Menu(weekdays, &day_selector, ft::MenuOption::HorizontalAnimated());
    menu = ft::Renderer(menu, [=, this] {
      if (day_selector == 0) {
        day_selector = weekdays.size() - ACTION_COUNT;
        timetable_contents->ChildAt(TIMETABLE_POSITION)->Detach();
        std::thread([=, this] {
          timetable_display(std::shared_ptr(timetable_component_for_lamda), api,
                            prev_url);
        }).detach();
...
ft::Component timetable::lessons(const std::vector<api::lesson_t> &day,
  ...
  for (auto &lesson : day) {
    ...
    if (lesson.is_empty) {
      if (events_component->ChildCount() >= 1)
        lessons->Add(
            ft::Container::Horizontal({empty_lesson_box(), events_component}));
      else
    ...
    if (events_component->ChildCount() >= 1)
      lessons->Add(
          ft::Container::Horizontal({lesson_component, events_component}));
    else
      lessons->Add(lesson_component);
  }
  return ft::Renderer(lessons,
                      [=, this] { return lessons->Render() | ft::yframe; });
}

ft::Component timetable::empty_lesson_box() {
  const auto seperator_size = ft::size(ft::WIDTH, ft::EQUAL, 20);
  return ft::MenuEntry(
      {.label = "", .transform = [=](const ft::EntryState &s) {
         return custom_ui::focus_managed_whatever(
             ft::vbox({ft::separatorEmpty(),
                       ft::separator() | seperator_size | ft::center,
                       ft::separatorEmpty()}),
...
ft::Component timetable::lesson_box(const api::lesson_t &lesson) {
  ...
  return ft::MenuEntry(
             {.label = lesson.subject,
              .transform =
                  [=, this](const ft::EntryState &s) {
                    if (lesson.is_canceled) {
                      return custom_ui::focus_managed_border_box(
                          ft::vbox(
                              {ft::text(s.label) | ft::bold |
                                   ft::color(
                                       config_p->Colors().get_accent_color1()) |
                                   ft::strikethrough | subject_size |
                                   ft::hcenter,
                               ft::text(lesson.start + deliminator +
                                        lesson.end) |
                                   ft::hcenter}) |
                              ft::xflex,
                          {.active = s.active, .focused = s.focused});
                      ...
                      ft::Elements base = {
                          ft::hbox({ft::text(s.label) | ft::bold,
                                    ft::text(" - Substitution")}) |
                              ft::color(
                                  config_p->Colors().get_accent_color2()) |
                              ft::hcenter,
                          ft::text(lesson.start + deliminator + lesson.end) |
                              ft::hcenter};

                      if (*show)
                        base.emplace(base.begin() + 1,
                                     ft::text(lesson.teacher) | ft::hcenter);
